...
class LimBinaryIOChunker(BaseChunker):
    def __init__(self, fh: typing.BinaryIO, 
        ...
        if self.is_readonly:
            ...
            self._version = self.readfileheader()
            ...
            if self.is_appending:
                ...
                self._version = (3, 0)
                self.writefileheader(self._version)
                # 2. init the map
    ...
    def readfileheader(self) -> tuple[int, int]|None:
    ...
    def writefileheader(self, version: tuple[int, int]) -> None:
        self._fh.seek(0)
    ...
    def _write_chunkmap(self, pos: int, chmap: collections.OrderedDict) -> bytes:
        map_len: int = 32 + 8
        for chunkname in chmap.keys():
            map_len += len(chunkname) + 2 * 8
        ...
        data : bytes = b''
        QQ = struct.Struct("QQ")
        for chunkname in reversed(sorted(chmap.keys())):
            data += chunkname + QQ.pack(*chmap[chunkname])
    ...
    def setBinaryRasterData(self, binid: int, seqindex: int, binimage: NumpyArrayLike) -> None:
        ...
        for y in range(0, binmeta.shape[0], binmeta.tileShape[0]):
            for x in range(0, binmeta.shape[1], binmeta.tileShape[1]):
                ...
                tile[0:yslice.stop-yslice.start, 0:xslice.stop-xslice.start] = binimage[yslice, xslice]
