...
ND2_MIN_DOWNSAMPLEDSIZE = 512

def _fullres_base_pow2(*shape) -> int:
    target = max(*shape)
    if target <= 1:
        return 0
...
def fullres_size(*shape) -> int:
    return 2 ** _fullres_base_pow2(shape)
...
class ImageAttributes:
    ...
    MinDownsampledSie: typing.ClassVar[int] = ND2_MIN_DOWNSAMPLEDSIZE
    ...
    def powSizeBase(self) -> int:
        return _fullres_base_pow2(self.uiWidth, self.uiHeight)
    
    @property
    def powSize(self) -> int:
        return fullres_size(self.uiWidth, self.uiHeight)
    
    @property
    def lowerPowSizeList(self) -> list[int]:
        ret, size = [], fullres_size(self.uiWidth, self.uiHeight)
        while ND2_MIN_DOWNSAMPLEDSIZE < size:
            size //= 2
            ret.append(size)
        return ret
    ...
    def makeDownsampled(self, downsize : int|None = None) -> ImageAttributes:
        fullsize = fullres_size(self.uiWidth, self.uiHeight)
        if downsize is None:
            downsize = fullsize // 2
        if downsize <= 2:            
            raise ValueError(f"Unexpected downsize: {downsize}")
        w = self.uiWidth * downsize // fullsize
        h = self.uiHeight * downsize // fullsize
