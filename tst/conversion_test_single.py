...
# Ensure tests import limnd2 from this repository (src/) instead of site-packages.
...
def runTest():
    if len(sys.argv) >= 2:           # if provided args, use those instead
        convert_sequence_to_nd2_cli(args=sys.argv[1:])
        return

    args = args9                     # select testing args list here
...
args7 = r"D:\files\tiff_files .*testWF780_Argolight_03_zstack-OME_TIFF-Export-11.* --extension tiff -n new.nd2 -o D:\files\tiff_files --pixel_calibration 0.103174604 --ms-objective_numerical_aperture 1.4 --ms-immersion_refractive_index 1.518 --multiprocessing --logs_to_json --channel-setting 0|DAPIa|Wide-field|353|465|#7C2A91 --channel-setting 1|AF488a|Wide-field|493|517|2E8B7A --channel-setting 2|AF555a|Wide-field|553|568|D98F3A --channel-setting 3|AF647a|Wide-field|653|668|Red"
args8 = r"D:\files\nd2_seq .*seq_m(.+?)\.nd2.* --multipoint 1 --extension nd2 -n seq_newnew.nd2 -o D:\files\nd2_seq --multiprocessing --logs_to_json"
# Flatten spectral test:
# "Time<number>" in filename is treated as additional channel axis from sequence.
args9 = r"D:\files\tiff_files\spectral_attempt Time(.+?)_Point0000\.tif --channel 1 --extension tif -n spectral_attempt_flatten.nd2 -o D:\files\tiff_files\spectral_attempt --flatten_duplicates --multiprocessing --logs_to_json"
