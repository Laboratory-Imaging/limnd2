...
def get_metadata(arguments: ConvertSequenceArgs,
    ...
    channel_minimal = {
        "microscope": {
          "immersionRefractiveIndex": arguments.metadata._other_settings["immersion_refractive_index"],
          "objectiveMagnification": arguments.metadata._other_settings["objective_magnification"],
          "objectiveNumericalAperture": arguments.metadata._other_settings["objective_numerical_aperture"],
          "pinholeDiameterUm": arguments.metadata._other_settings["pinhole_diameter"],
          "zoomMagnification": arguments.metadata._other_settings["zoom_magnification"]
        },
        "volume": {
          "axesCalibrated": [
            True,
          ...
          "axesCalibration": [
            arguments.metadata.pixel_calibration,
            arguments.metadata.pixel_calibration,
    ...
    if "channel" not in exp_count:
        ...
        for key in sorted(grouped_files):
            if arguments.channels:
                ...
                group_files = [file[1].name for file in sorted(grouped_files[key].items(), key=lambda x: channel_order.index(x[0]))]
            else:
                # if channels were not provided, sort files in group based on channel name
                group = grouped_files[key]
                group_files = []
                for group_key in sorted(group):                 # TODO heres how files are sorted, right now they are sorted alphabetically, sort them using channel list instead
